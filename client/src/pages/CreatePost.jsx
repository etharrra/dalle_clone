...
const CreatePost = () => {
	...
	const generateImage = async () => {
		if (form.prompt) {
			try {
				...
				const response = await fetch("https://dalle-clone-th.onrender.com/api/v1/dalle", {
					method: "POST",
					headers: {
	...
	const handleSubmit = async (e) => {
		...
		if (form.name && form.prompt && form.photo) {
			setLoading(true);
			try {
				const response = await fetch("https://dalle-clone-th.onrender.com/api/v1/post", {
					method: "POST",
					headers: {
						"Content-Type": "application/json",
					},
			...
			<form action="" className="mt-16 max-w-3xl" onSubmit={handleSubmit}>
				<div className="flex flex-col gap-5">
					...
					<FormField
						...
						isSuspriseMe
						handelSuspriseMe={handelSuspriseMe}
					/>
